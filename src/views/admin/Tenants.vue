<template>
  <div class="pt-2 pb-40 md:py-6 mx-auto px-4 sm:px-6 md:px-8">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in orderedItems" :key="index">
              <!-- <td
                ...
                class="truncate px-3 py-2 text-sm leading-3 font-medium text-gray-800 hover:text-theme-700 cursor-pointer"
              >{{ item.name }}</td>-->
              ...
              >{{ item.subdomain }}</td>-->
              <!-- <td
                class="truncate px-3 py-2 text-center text-sm leading-3 font-medium text-gray-900"
              >
                <img
                  v-if="item.icon"
              ...
              <!-- <td
                class="truncate px-3 py-2 text-center text-sm leading-3 font-medium text-gray-900 bg-gray-800"
              >
                <img
    ...
    <modal ref="imageModal" class="bg-primary">
      <div
        class="rounded-sm pb-4 pt-4"
...
export default class TenantsComponent extends BaseComponent {
  ...
  changeTenant(tenant: TenantDto) {
    this.services.users.updateDefaultTenant(tenant.id ?? "").then(response => {
      // ignore
    });
  }
