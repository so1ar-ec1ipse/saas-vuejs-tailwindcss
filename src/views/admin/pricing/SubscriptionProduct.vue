  ...
  <div>
    ...
    <div v-else>
      <div v-if="item && item.id">
          ...
          <template v-slot:buttons>
            <span class="sm:ml-3 rounded-sm">
              ...
              <button
                v-if="hasChanges"
                @click="reload"
                class="ml-2 inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-sm text-gray-700 bg-yellow-50 hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:text-gray-800 active:bg-gray-50 transition duration-150 ease-in-out"
              >
                {{ $t("shared.cancel") }}
              </button>
              <button
        ...
        <div class="hidden sm:block mb-3 mt-4 sm:mt-9">
          <nav class="flex">
            <!-- <router-link
