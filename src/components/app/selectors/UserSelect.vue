    ...
    <div v-click-outside="closeDropDown">
      ...
      <div class="mt-1 relative">
        ...
        >
          ...
          <span
            v-if="selectedTenantUser"
            class="flex items-center"
            v-else-if="selected > 0"
          >
            <img
              v-if="selectedTenantUser.user.avatar"
              :src="selectedTenantUser.user.avatar"
              alt="User"
              class="flex-shrink-0 h-6 w-6 rounded-full"
            />
            <span
              v-else
              class="flex-shrink-0 inline-flex items-center justify-center h-6 w-6 rounded-full bg-gray-500"
            >
              <span class="text-xs font-medium leading-none text-white">{{
                avatarText(selectedTenantUser)
              }}</span>
            </span>
            <span class="ml-3 block truncate">
              {{ name(selectedTenantUser) }}
            </span>
