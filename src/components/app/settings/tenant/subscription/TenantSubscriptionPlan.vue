      ...
      <div class="shadow sm:rounded-sm">
            ...
            <div v-else class="col-span-6 block">
              ...
              <div v-else v-for="(myProduct, idx) in myProducts" :key="idx">
                <div>
                  ...
                  <span
                    class="-mt-4 block text-right text-medium text-sm text-theme-500 font-medium"
                    >{{ myProduct.subscriptionProduct.title }} ${{
                      myProduct.subscriptionPrice.price
                    }}
                    {{
                      myProduct.subscriptionPrice.currency.toUpperCase()
                    }}</span
                  >
                </div>
              </div>
        ...
        <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
          <button
            ...
            class="mx-2 py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-sm text-white bg-theme-600 shadow-sm hover:bg-theme-500 focus:outline-none focus:shadow-outline-blue active:bg-gray-600 transition duration-150 ease-in-out"
          >
            <span>
          ...
          >
            <span>
              {{
                !editing
                  ? $t("marketing.pricing.updateSubscription")
                  : $t("shared.cancel")
              }}
            </span>
          </button>
...
export default class TenantSubscriptionPlan extends BaseComponent {
  ...
  updateSubscription() {
    if (!this.selectedPrice || !this.selectedPrice.serviceId) {
      // @ts-ignore
